...
public sealed class StudiosListSettings : GlobalSettings
{
    [CommandOption("--search <TERM>")]
    [Description("Filter studios by name")]
...
public sealed class StudiosListCommand : ApiCommand<StudiosListSettings>
{
    public StudiosListCommand(ApiClientFactory clientFactory, CredentialStore credentialStore)
        : base(clientFactory, credentialStore)
    {
    }

    protected override async Task<int> ExecuteAsync(
        CommandContext context, StudiosListSettings settings, JellyfinApiClient client, CancellationToken cancellationToken)
    {
        var userId = await ResolveOptionalUserIdAsync(settings, client, cancellationToken);
        var result = await client.Studios.GetAsync(config =>
        {
        ...
        return ItemsCommandOutputHelper.WriteBaseItemQueryResult(settings, result, "No studios found.");
    }
